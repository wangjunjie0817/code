package com.wang.code.xiangxue.threadPool.schd;
...
public class ScheduleWorkerTime implements Runnable{
    ...
    public void run() {
    	if(count.get()==0) {
			...
			try {
				TimeUnit.MILLISECONDS.sleep(Long_8);
			...
			try {
				TimeUnit.MILLISECONDS.sleep(Short_2);
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
			...
			try {
				TimeUnit.MILLISECONDS.sleep(Normal_5);
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
